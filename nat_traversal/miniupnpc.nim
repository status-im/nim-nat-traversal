...
when defined(miniupnpcUseSystemLibs):
  ...
  import os
  const rootPath = currentSourcePath.parentDir().parentDir().replace('\\', '/')
  const miniupnpcPath = rootPath & "/vendor/miniupnp/miniupnpc"
  const includeFlag = "-I" & miniupnpcPath & "/include"
  const libraryPath = miniupnpcPath & "/build/libminiupnpc.a"
  {.passc: includeFlag.}
