...
proc compileStaticLibraries() =
  withDir "vendor/miniupnp/miniupnpc":
    when defined(windows):
      exec("mingw32-make -f Makefile.mingw SHELL=sh CC=gcc init libminiupnpc.a")
    else:
      exec("make libminiupnpc.a")
  withDir "vendor/libnatpmp":
    when defined(windows):
      exec("mingw32-make SHELL=sh CC=gcc CFLAGS=\"-Wall -Os -DWIN32 -DNATPMP_STATICLIB -DENABLE_STRNATPMPERR\" libnatpmp.a")
    else:
